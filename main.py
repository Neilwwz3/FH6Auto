...
class FH_UltimateBot(ctk.CTk):
    ...
    CJ_IN_CAR_DEEP_PAGE_IDS = frozenset(
    ...
    def _cj_page_label(self, page_id):
        extra_labels = {
            "BUY_SELL_HUB": "购买与出售(车展/拍卖场页)",
            "VEHICLE_TAB": "嘉年华-车辆标签",
        }
    ...
    def _cj_probe_templates_in_regions(self, templates, region_keys):
        """在指定区域键列表中取模板最高灰度分。"""
        best = 0.0
        seen_regs = []
        for key in region_keys:
            region = self.regions.get(key)
            if region is None or region in seen_regs:
                continue
            seen_regs.append(region)
            for p in self.probe_gray_templates(templates, region=region, fast_mode=True):
                if not p.get("missing"):
    ...
    def _cj_probe_vehicle_tab_score(self):
        ...
        return self._cj_probe_templates_in_regions(
            [self.CJ_VEHICLE_TAB_TEMPLATE], ("左",)
        )
    ...
    def _cj_is_vehicle_tab_confirmed(self):
        ...
        score = self._cj_probe_vehicle_tab_score()
    ...
    def _cj_ensure_vehicle_tab(self, max_pagedown=12):
        ...
        for step in range(max_pagedown):
            if not self.is_running:
                return False
            ok, score = self._cj_is_vehicle_tab_confirmed()
            self.log(
                f"[车辆标签] 步骤 {step + 1}: tab={score:.3f} "
                f"(阈值 {self.CJ_VEHICLE_TAB_THRESHOLD}) | "
                f"{'已确认' if ok else '继续 PageDown'}"
            )
            if ok:
                return True
        ...
        self.log_image_detection_failure(
            "嘉年华-车辆标签(UandT-w)",
            [self.CJ_VEHICLE_TAB_TEMPLATE],
            region=self.regions.get("左", self.regions["全界面"]),
            threshold=self.CJ_VEHICLE_TAB_THRESHOLD,
            extra=f"PageDown {max_pagedown} 次后仍未确认；末次 tab={score:.3f}",
        )
        return False
    ...
    def detect_current_page_cj(self, log_scores=False):
        ...
        all_scores["VEHICLE_TAB"] = {
            ...
            "label": "嘉年华-车辆标签",
            "probes": [],
            "tab_confirmed": tab_ok,
        }
        carnival_active = hub_ok or tab_ok
        carnival_floor = max(
            hub_score if hub_ok else 0.0,
            tab_score if tab_ok else 0.0,
            hub_score * 0.85,
            tab_score * 0.85,
        )
        ...
        for page_def in self.CJ_PAGE_DEFS:
            ...
            if page_def["id"] in self.CJ_IN_CAR_DEEP_PAGE_IDS and carnival_active:
                if page_best <= carnival_floor + self.CJ_CARNIVAL_SUPPRESS_MARGIN:
                    continue
            if page_best >= page_def["threshold"] and page_best > best_score:
                best_score = page_best
                best_id = page_def["id"]

        if tab_ok and tab_score > best_score:
            best_score = tab_score
            best_id = "VEHICLE_TAB"
        if hub_ok and hub_score > best_score:
    ...
    def _cj_enter_my_cars_brand_list(self):
        """从「购买与出售」进入「我的车辆」→ Backspace 切到按制造商浏览。"""
    ...
    def _cj_select_subaru_brand(self, max_probe=12):
        """
        制造商列表：Up 一下可跳到列表底部（斯巴鲁在下方），再识图 CCbrand 点击。
        返回是否成功选中。
        """
        if not self.is_running:
            return False
        ...
        for step in range(max_probe):
            if not self.is_running:
                return False
            brand_pos = self.find_any_image_gray(
                ["CCbrand.png"],
                region=region,
                threshold=threshold,
                fast_mode=True,
            ...
            if brand_pos:
                self.log(f"找到斯巴鲁品牌（Up 后第 {step + 1} 次识图）")
                break
            time.sleep(0.15)
        ...
        if not brand_pos:
            ...
            self.log_image_detection_failure(
                ...
                extra=(
                    "已按 Up 拉到底部仍无命中；"
                    f"当前页面={self._cj_page_label(cur_page)}({cur_score:.2f})"
                ),
            )
            return False
    ...
    def _cj_nav_step_toward(self, current_id, target_id):
        ...
        if target_id == "MY_CARS_LIST":
            if current_id == "VEHICLE_TAB":
                self._cj_enter_my_cars_brand_list()
                return False
            if current_id == "BUY_SELL_HUB":
                if self._cj_ensure_vehicle_tab(max_pagedown=8):
                    self._cj_enter_my_cars_brand_list()
                return False
            if current_id in self.CJ_IN_CAR_DEEP_PAGE_IDS:
                ...
                if hub_side or tab_side:
                    self.log(
                        f"[页面导航] 仍在嘉年华流程(hub={hub_side} tab={tab_side})，"
                        f"「{self._cj_page_label(current_id)}」疑为误识别 → 进入我的车辆"
                    )
                    if tab_side:
                        self._cj_enter_my_cars_brand_list()
                    elif self._cj_ensure_vehicle_tab(max_pagedown=8):
                        self._cj_enter_my_cars_brand_list()
                    return False
    ...
    def _cj_warn_if_unexpected_page(self, step_label, forbidden_page_ids):
        """
        仅作告警：若明确识别到不该出现的已标定页面则打日志。
        UNKNOWN 或未标定界面不触发告警、不驱动恢复。
        """
        page_id, score, _ = self.detect_current_page_cj(log_scores=False)
        if page_id == "UNKNOWN" or page_id not in forbidden_page_ids:
            return
        self.log(
            f"[页面告警] 步骤「{step_label}」时出现在非预期页面: "
    ...
    def cj_recover_to_checkpoint(self, checkpoint_id="MY_CARS_LIST", max_rounds=2):
        ...
        if checkpoint_id != "MY_CARS_LIST":
            self.log(f"[页面恢复] 未实现的检查点 {checkpoint_id}")
            return False
    ...
    def _cj_find_upgrade_tuning_entry(self, boarded_via_rc):
        """在车辆详情中寻找「升级与调教」（原逻辑：识图 + ESC，不接入 PageDef）。"""
        ...
        if not pos_sjy:
            self.log_image_detection_failure(
                ...
                region=self.regions["左下"],
                threshold=ut_threshold,
                extra=f"上车方式={'rc点击' if boarded_via_rc else 'Enter'}",
            )
        return pos_sjy
        ...
        while self.cj_counter < target_count:
            ...
            for _ in range(85 - jump_pages):
                ...
                pos_target = self.wait_for_image_with_element_multi(
                    ...
                    "newcartag.png",
                    region=self.regions["全界面"],
                    main_threshold=0.70,   # 防HDR：与 upstream 一致
                    like_threshold=0.70,
                    final_threshold=0.70,
                    timeout=1.5,
                    interval=0.2,
